...
getgenv().Settings = {
    ['Made By JoJo#2494'] = {
        CHECK = {OWNER = "K8U8X0XGFHWE", STAND = "K8U8X0XGFHWE"}, --/ Do not use UserId.
        ...
        TITAN = {ENABLED = false, TALL = false, WIDE = false, GODV3 = false}, --/ If enabled you will become a titan stand, if you enable god you need to execute before load for it to work (also can be used with titan disabled).
...
if game:GetService("Players").LocalPlayer.Name == (Settings['Made By JoJo#2494'].CHECK.STAND) then  
    loadstring(game:HttpGet("https://raw.githubusercontent.com/JOJOGIO/STAND-FRAMEWORK/main/1.0.6"))()
  ...
  if Settings['Made By JoJo#2494'].TITAN.ENABLED then
    coroutine.wrap(function()
      repeat wait() until STAND.Backpack:FindFirstChild("Mask")
        ...
        if Settting.TITAN.TALL then
            if OWNER.Character.Humanoid:FindFirstChild("BodyDepthScale") then
                rm()
                wait(0.6)
                OWNER.Character.Humanoid:FindFirstChild("BodyDepthScale"):Destroy()
                wait(0.6)
            end
            if OWNER.Character.Humanoid:FindFirstChild("BodyTypeScale") then
                rm()
                wait(0.6)
                OWNER.Character.Humanoid:FindFirstChild("BodyTypeScale"):Destroy()
                wait(0.6)
            end
        end
        if Settings.TITAN.WIDE then
...
--//------------------------------------------------------------------------------------------\\--
Notify("JoJo's Stand Framework 1.0.6","Success!")
end
--//------------------------------------------------------------------------------------------\\----//------------------------------------------------------------------------------------------\\--
